...
try{
 ...
 await page.selectOption('#language','es');await page.getByText('Sigue el intercambio.',{exact:true}).waitFor();assert.equal(await page.locator('html').getAttribute('lang'),'es');assert.equal(await page.locator('#log .event').count(),transcriptCount);await page.screenshot({path:fileURLToPath(new URL('spanish.png',artifacts)),fullPage:true});
 await page.selectOption('#language','en');await page.getByText('Follow the handoff.',{exact:true}).waitFor();assert.equal(await page.locator('#log .event').count(),transcriptCount);report.checks.push('EN/ES switches locally without resetting the transcript');
 ...
 for(const [name,width,height] of [['ipad',1024,1366],['mobile',390,844]]){await page.setViewportSize({width,height});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Horizontal overflow');await page.screenshot({path:fileURLToPath(new URL(`${name}.png`,artifacts)),fullPage:true})}
 ...
 await page.locator('#log .event h3').filter({hasText:/^Parent settled/}).waitFor();assert.ok((await page.locator('#child').innerText()).includes('child-1'));await page.waitForTimeout(300);
 await page.screenshot({path:fileURLToPath(new URL('live-test-feed.png',artifacts)),fullPage:true});
 await page.reload();await page.locator('#log .event h3').filter({hasText:/^Parent settled/}).waitFor();assert.equal(await page.locator('#log .event').count(),8);
