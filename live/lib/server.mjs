...
export async function startObserver({port=0, mode='live', assets={}, producerPid, benchmarkDirectory}={}) {
  const key=randomBytes(24).toString('hex'), clients=new Set(), history=[];
  let sequence=0;
  const ingestKey=randomBytes(24).toString('hex');
  let publish,boundPort,closing=false,closePromise,benchmarkTimer;
  const broadcastBenchmark=()=>{benchmarkTimer=undefined;for(const res of clients)if(!res.write(`event: benchmarks\ndata: ${JSON.stringify(benchmarks.snapshot())}\n\n`)){res.destroy();clients.delete(res)}};
  const benchmarks=await createBenchmarkStore({directory:benchmarkDirectory,onChange:()=>{if(!closing&&!benchmarkTimer){benchmarkTimer=setTimeout(broadcastBenchmark,250);benchmarkTimer.unref()}}});
  const server=http.createServer(async (req,res)=>{
    ...
    if(url.pathname==='/ingest' && mode==='workspace') {
      ...
      try {
        ...
        benchmarks.markTransportLoss(body.telemetryDropped);publish(e);res.writeHead(204).end();
    ...
    if(url.pathname==='/benchmarks'){
      ...
      res.writeHead(200,{...headers,'Content-Type':'application/json'}).end(JSON.stringify(benchmarks.snapshot()));return;
    }
    ...
    const files={'/':'index.html','/app.js':'app.js','/style.css':'style.css','/i18n.js':'i18n.js','/terminal.js':'terminal.js','/benchmarks.js':'benchmarks.js','/benchmark-math.js':'benchmark-math.js'};
  ...
  publish=event=>{if(!event||closing)return;if(event.kind==='benchmark_observation'){if(mode!=='demo'&&benchmarks.consume(event.metrics)&&!benchmarkTimer){benchmarkTimer=setTimeout(broadcastBenchmark,250);benchmarkTimer.unref()}return;}const row={...event,id:++sequence,time:new Date().toISOString(),mode};history.push(row);if(history.length>500)history.shift();for(const res of clients)if(!res.write(packet(row))){res.destroy();clients.delete(res)}};
  return {
    server,benchmarks,
    ...
    close(){
      ...
      closing=true;clearInterval(heartbeat);clearTimeout(benchmarkTimer);for(const res of clients)res.end();clients.clear();
      closePromise=Promise.all([benchmarks.close(),new Promise((resolve,reject)=>server.close(error=>error?reject(error):resolve()))]).then(()=>undefined);
      return closePromise;
    }
  };
}
